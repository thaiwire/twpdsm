...
| Role | ขอบเขตการดู | สร้าง/แก้ไขเอกสาร | ลบเอกสาร | อนุมัติเอกสาร | เข้า `/admin/*` |
|---|---|---|---|---|---|
| **ADMIN** | ทุกหน่วยงาน | ได้ทุกหน่วยงาน | ได้เสมอ แม้เอกสารอนุมัติแล้ว | ได้ทุกหน่วยงาน | ได้ |
| **MANAGER** | หน่วยงานตัวเอง + สิทธิ์พิเศษที่ได้รับ | เฉพาะหน่วยงานตัวเอง | หน่วยงานตัวเอง — ลบได้แม้เอกสารอนุมัติแล้ว | เฉพาะหน่วยงานตัวเอง | ไม่ได้ |
| **STAFF** | หน่วยงานตัวเอง + สิทธิ์พิเศษที่ได้รับ | เฉพาะหน่วยงานตัวเอง | หน่วยงานตัวเอง — **ลบไม่ได้ถ้าเอกสารอนุมัติแล้ว** | ไม่ได้ | ไม่ได้ |
...
เอกสารกลายเป็นสถานะ "อนุมัติแล้ว" (แสดง badge เขียว "✓ อนุมัติแล้ว โดย [ชื่อ] เมื่อ [วันที่]")
        │
        ▼
จากนี้ไป: canDeleteDocument() จะปฏิเสธการลบ ยกเว้น
  - ADMIN (ลบได้เสมอ)
  - MANAGER ของหน่วยงานเดียวกับเอกสาร
  STAFF ธรรมดา (แม้เป็นคนสร้างเอกสารเอง) ลบไม่ได้อีกต่อไป — ปุ่มลบหายไปจาก UI
  และถ้าพยายาม bypass ผ่าน server action ตรงๆ จะได้ error message อธิบายเหตุผล
```

**ขอบเขตของการล็อก**: ครอบคลุมทั้ง "ลบทั้งเอกสาร" และ "ลบไฟล์แนบรายไฟล์" — ไม่ใช่แค่ตัวเอกสาร

**ไม่มีการ "ยกเลิกอนุมัติ"** — เป็นการอนุมัติทางเดียว (one-way gate) ตรงตามที่ต้องการคือ "อนุมัติแล้วลบไม่ได้"

---

## 9. Flow: การลบเอกสาร / ไฟล์แนบ
...
## 10. Flow: การดาวน์โหลด/พรีวิวไฟล์
...
## 11. Flow: การจัดการข้อมูลอ้างอิง (Admin)
...
## 12. Flow: ประวัติการใช้งาน (Audit Log)

```
ทุกครั้งที่มีการ CREATE / DOWNLOAD / DELETE / APPROVE เอกสารหรือไฟล์แนบ
        │
        ▼
...
**คุณสมบัติเด่น**: log ยังอ่านได้ครบแม้เอกสารต้นฉบับถูกลบไปแล้ว (ดูเหตุผลในข้อ 9) — ตอบโจทย์ "ตรวจสอบว่าใคร add/delete/download เอกสาร" ได้แม้เอกสารนั้นไม่มีอยู่แล้ว

---

## 13. Flow: โปรไฟล์ผู้ใช้ (Profile)
...
## 14. รายการฟีเจอร์ทั้งหมด (สรุปย่อ)
...
| **อนุมัติเอกสาร (ล็อกการลบ)** | [ApproveButton.tsx](src/components/ApproveButton.tsx) |
...
| `DocumentAudit` | ประวัติการใช้งาน — CREATE/DOWNLOAD/DELETE/APPROVE พร้อม snapshot |
